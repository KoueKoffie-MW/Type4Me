...
fun MainScreenContent(
    ...
    Scaffold(
        ...
        Column(
            modifier = Modifier
                ...
                .then(if (isLandscape) Modifier.verticalScroll(rootScrollState) else Modifier),
            ...
            when (state.activeMode) {
                ...
                AppMode.SNIPPETS -> {
                    SnippetsPadScreen(
                        ...
                        modifier = if (isLandscape) Modifier.fillMaxWidth().heightIn(min = 400.dp) else Modifier.fillMaxSize()
                    )
                }
    ...
    val promptTitle = state.activePromptSnippet?.title ?: state.activePromptMacro?.title
    if (promptTitle != null && state.activePrompts.isNotEmpty()) {
